...
interface BookData {
  ...
  description: string;
  pages: number;
  category: 'primary' | 'junior-high' | 'senior-high';
}

// Structured book data from your catalogue
...
async function importBooks() {
  ...
  for (const [index, book] of booksData.entries()) {
    try {
      ...
      // Create document via Management API
      const response = await fetch(`https://maybridgepublishing.cdn.prismic.io/api/v2/documents`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error(`❌ Failed to import ${book.title}:`, errorText);
        errorCount++;
      }
    } catch (error) {
